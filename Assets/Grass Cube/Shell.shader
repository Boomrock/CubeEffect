Shader "Unlit/NewUnlitShader"
{
    Properties
    {
	    ...
	    _ShellDistanceAttenuation ("Shell Distance Attenuation", Float) = 1.0
	    _Curvature ("Curvature", Float) = 1.0
	    _DisplacementStrength ("Displacement Strength", Float) = 1.0
	    _ShellColor ("Shell Color", Color) = (1.0, 1.0, 1.0)
	    _ShellDirection ("Shell Direction", Vector) = (0.0, 1.0, 0.0)
    }
    SubShader
    {
        ...
        Pass
        {
           ...
           // Unity has a lot of built in useful graphics functions, all this stuff is on github which you can look at and read there aren't really any
			...
			float4 frag(v2f i) : SV_TARGET {
                ...
                const float height = tex2D(_HeightMap, i.uv).r;
                ...
                const float rand = lerp(_NoiseMin, _NoiseMax, hash(seed))*height;

				// This is the normalized shell height as described above in the vertex shader
                const float h = shellIndex / shellCount;
                ...
                const int outsideThickness = (localDistanceFromCenter) > (_Thickness * (rand - h));
				...
				_ShellColor = _ShellColor*_ShellIndex/(_ShellIndex+1);
                ...
                return float4(_ShellColor * ndotl * ambientOcclusion, col.x);
			}
            ENDCG
        }
    }
